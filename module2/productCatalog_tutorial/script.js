...
function getIntersection(arrA, arrB, searchedId, type){
...
function processSearch(searchId, name) {
    if (name === "id") {
        api.searchProductById(searchId).then(function(val){
            ...
            var similarArray = getIntersection(val[0],val[1],val[2].id, name);
    ...
    if (name === "price") {
        api.searchProductsByPrice(searchId, 10).then(function(val){
            updateTable("similarTable", val);
        }).catch(function(val){
            alert(val);
        });  
...
var elements = document.getElementsByClassName("inputButton");

Array.prototype.forEach.call(elements, function (el) {
    el.addEventListener("click", function(e) {
        var name = e.srcElement.id;
        processSearch(document.getElementById("input-" + name).value, name);  
    }); 
});
